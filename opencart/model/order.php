...
class ModelAccountOrder extends Model {
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'post_date'               => $order_query->row['post_date'],
				'ip'                      => $order_query->row['ip'],
                'ttn'                     => $order_query->row['ttn'],
                'ttn_status'              => $order_query->row['ttn_status'],
                'sum_'                    => $order_query->row['sum_'],
                'sum_date'                => $order_query->row['sum_date'],
	...
	public function getOrders($start = 0, $limit = 20, $order_status = 0) {
		if ($start < 0) {
			$start = 0;
		}
        ...
        $sql = "SELECT o.order_id, o.firstname, o.lastname, o.shipping_firstname, o.shipping_lastname, os.name as status, o.date_added, o.total, o.order_status_id, o.currency_code, o.currency_value,  o.ttn, o.ttn_status, o.sum_, o.sum_date FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND";
        if((int)$order_status == 0){
            $sql .= " o.order_status_id > '0'";
        }else{
            $sql .= " o.order_status_id = '" . (int)$order_status . "'";
        }
        $sql .= " AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit;
